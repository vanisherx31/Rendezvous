...
rT=[50 50]      %%%%%%%%% target position in x and y direction
rTx=rT(1);
rTy=rT(2);

t=20;

vR=[5 5];     %%%%%%%%% robot initial velocity vR in x and y direction
vRx=vR(1);
vRy=vR(2);
magvR=sqrt((vRx)^2+(vRy)^2);

magvT=190;    %%%%%%%%%%% defining target velocity vT magnitude and angle
thetaT=80;
...
vRTmaxx=vRT(1)+(magvrendmaxrel/dRT)*(rT(1)-rR(1));
vRTmaxy=vRT(2)+(magvrendmaxrel/dRT)*(rT(2)-rR(2));
...
pR=plot(rRx,rRy,'o','Markerfacecolor','r','Markeredgecolor','g','Markersize',10);hold on;
pT=plot(rTx,rTy,'o','Markerfacecolor','b','Markeredgecolor','g','Markersize',10);hold on;
...
pvRTmax=plot(vRTmax(1),vRTmax(2),'^','Markerfacecolor','b','Markeredgecolor','b','Markersize',8)
...
if (isempty(SP1)~=1)
pSP1=plot(SP1(1),SP1(2),'o','Markerfacecolor','g','Markeredgecolor','g','Markersize',8);hold on;
else
end

if (isempty(SP2)~=1)
pSP2=plot(SP2(1),SP2(2),'o','Markerfacecolor','g','Markeredgecolor','g','Markersize',8);hold on;
else
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
